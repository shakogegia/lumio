...
- **Conductor dev workspaces:** the lifecycle scripts make auth work per-workspace without manual edits. `scripts/conductor/setup.sh` generates a strong `BETTER_AUTH_SECRET` into `.env` when it's missing or still a placeholder. `scripts/conductor/run.sh` sets `BETTER_AUTH_URL` to the origin the browser actually uses, so `trustedOrigins` matches (otherwise sign-in fails the origin/CSRF check, `INVALID_ORIGIN`). After merging main's **per-workspace portless proxy** (workspaces served at `https://<workspace>.lumio.localhost:1355`), `run.sh` sets `BETTER_AUTH_URL` to that subdomain when portless is active and adds the direct `http://localhost:<port>` origin via `BETTER_AUTH_TRUSTED_ORIGINS` so both access paths work; on plain localhost it falls back to `http://localhost:$PORT`. `auth.ts` merges `BETTER_AUTH_URL` + the comma-separated `BETTER_AUTH_TRUSTED_ORIGINS` into `trustedOrigins` (deduped). (`dotenv-cli` does not override an already-exported var, so the runtime exports win over `.env`.)
