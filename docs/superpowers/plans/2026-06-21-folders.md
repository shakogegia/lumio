...
**Baseline (post-rebase onto `main`):** this plan integrates with the recently-merged **album-cover** feature (#58). Concretely: `Album` already has a `coverPhotoId` column; `albums-service.ts` `listAlbumSummaries` already resolves a pinned-or-derived cover (Task 7 must preserve that logic when extracting `albumSummary`); `/api/albums/[id]` already exports a `PATCH` that sets the cover (Task 15 **extends** it for rename rather than adding a second handler); the cover migration `20260621100454_add_album_cover_photo_id` already exists (the Task 3 folders migration `20260621120000_add_folders` sorts after it). Do not remove or overwrite any cover functionality.
...
Add the `folderId` column + relation + index. The `Album` model already carries `coverPhotoId` (from the album-cover feature on main — keep it). The full model becomes:
...
Replace the existing `listAlbumSummaries` function with an extracted, reusable `albumSummary` plus a thin list wrapper. **Preserve the cover-pinning logic exactly** (the album-cover feature on main: a pinned `coverPhotoId` is honored only while it is still a member, otherwise the most-recent member is derived) — `albumSummary` must contain the same body that currently lives inline in `listAlbumSummaries`:
...
- Modify: `apps/web/src/app/api/albums/[id]/route.ts` (**extend the existing `PATCH`** — it already handles set-cover from the album-cover feature; add rename to it)
...
- [ ] **Step 2: Extend the EXISTING `PATCH` in `apps/web/src/app/api/albums/[id]/route.ts`**

This route already exports a `PATCH` that sets the album cover (`setAlbumCoverSchema` → `setAlbumCover`). Do **not** add a second `PATCH` export (Next.js allows only one per method). Instead, make the handler branch: a body with `name` → rename; otherwise → set cover.

Update the imports — add `renameAlbumSchema` to the shared import and `renameAlbum` to the service import (keep the existing `setAlbumCoverSchema`, `setAlbumCover`, `PhotoNotInAlbumError`, etc.):
...
Replace the existing `PATCH` handler with this branching version:
...
(`renameAlbumSchema` requires `name`, so a cover-only body `{ coverPhotoId }` fails the rename parse and falls through to the cover branch — no ambiguity.)
