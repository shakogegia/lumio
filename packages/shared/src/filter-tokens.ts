...
import { type FieldDef, ValueType, type FilterRule, type FilterValue, resolveField } from "./filters.js";
...
function coerceScalar(def: FieldDef, op: RuleOp, raw: string): FilterValue | undefined {
  ...
  if (def.type === ValueType.string && COMPARISON_OPS.has(op)) {
    const n = toNumber(v);
...
function tokenToRule(token: string): FilterRule | null {
  ...
  if (valuePart === "?") {
    ...
    op = def.type === ValueType.string ? RuleOp.contains : RuleOp.eq;
    raw = valuePart;
  }
