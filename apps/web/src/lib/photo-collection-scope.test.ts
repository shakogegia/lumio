...
describe("collectionForScope", () => {
  it("library: slug-scoped /photos with sort, base /c/fam/photos", () => {
    ...
    expect(c.baseUrl).toBe("/c/fam/photos");
    expect(c.urlForId("p1")).toBe("/c/fam/photo/p1"); // default sort omitted in URL
  });

  it("album: slug-scoped endpoint + ?album in the catalog-scoped URL", () => {
    ...
    expect(c.baseUrl).toBe("/c/fam/albums/alb1");
    expect(c.urlForId("p1")).toBe("/c/fam/photo/p1?album=alb1&sort=imported-asc");
  });

  it("search: slug-scoped /search with repeated album + q, base /c/fam/search", () => {
    ...
    expect(c.baseUrl).toBe("/c/fam/search");
    expect(c.urlForId("p1")).toBe("/c/fam/photo/p1?s=1&album=a&album=b&q=cat");
  });
});
